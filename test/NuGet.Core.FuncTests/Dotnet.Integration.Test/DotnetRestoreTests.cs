...
EndGlobal";
        ...
        [Fact]
        public async Task DotnetRestore_TouchMetadata_LastAccessTime()
        {
            using (var pathContext = _msbuildFixture.CreateSimpleTestPathContext())
            {
                ...
                var cacheExpiration = new XElement(XName.Get("add"));
                cacheExpiration.Add(new XAttribute(XName.Get("key"), "cacheExpiration"));
                cacheExpiration.Add(new XAttribute(XName.Get("value"), "true"));
                config.Add(cacheExpiration);
