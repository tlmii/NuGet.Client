...
{
    public class FallbackCertificateBundleX509ChainFactoryTests : IClassFixture<CertificatesFixture>, IDisposable
    {
        ...
        public void TryCreate_WhenFilePathIsRelative_SetsFullFilePath()
        {
            using (X509Certificate2 certificate = _fixture.GetDefaultCertificate())
            {
                ...
                var subdirectory = new DirectoryInfo(Path.Combine(currentDirectory, Path.GetRandomFileName()));
        ...
        [Fact]
        public void Create_WhenFileIsEmpty_ReturnsX509Chain()
        {
        ...
        [Fact]
        public void Create_WhenFileIsValid_ReturnsX509Chain()
        {
            using (X509Certificate2 certificate = _fixture.GetDefaultCertificate())
            {
        ...
        {
            var file = new FileInfo(Path.Combine(directoryPath, FallbackCertificateBundleX509ChainFactory.FileName));
